...
export default function ProductPage({ params }: Props) {
  ...
  const waMsg = encodeURIComponent(`Hi SĀRI! I'm interested in: ${product.name} (${formatPrice(product.sale_price ?? product.price)}). Can you help me?`);
  ...
  const accordions = [
    ...
    },
    {
      id: 'fabric',
      title: 'FABRIC & CARE',
      content: `Fabric: ${product.fabric}\n\nDry clean recommended. Store in a cool, dry place away from direct sunlight. Avoid prolonged contact with perfume. Handle zari with care.`,
    },
    {
      id: 'delivery',
      title: 'DELIVERY & RETURNS',
      content: 'Pan-India delivery in 5–7 business days. Express delivery available. Returns accepted within 7 days of delivery for unused items in original packaging. Bridal sarees are non-returnable.',
    },
  ];
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-12 py-8 lg:py-16">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-20">
              ...
              <div className="flex-1 relative overflow-hidden bg-cream aspect-[3/4] cursor-zoom-in" onClick={() => setLightbox(true)}>
                ...
                {product.stock <= 3 && product.stock > 0 && (
                  <div className="absolute top-4 left-4 bg-burgundy px-3 py-1">
                    <p className="label text-ivory text-[9px]">ONLY {product.stock} LEFT</p>
                  </div>
                )}
              </div>
            ...
            <div className="lg:pt-4">
              <p className="label text-gold text-[10px] mb-3">{product.category?.toUpperCase()}</p>

              <h1 className="editorial-heading text-espresso text-4xl lg:text-5xl mb-4 leading-tight">
                {product.name}
              </h1>
              ...
              <div className="flex items-center gap-2 mb-6">
                <div className="flex gap-0.5">
                  {Array(5).fill(0).map((_, i) => (
                    <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="#A88955" className="text-gold">
              ...
              <div className="flex items-baseline gap-3 mb-8">
                {product.sale_price ? (
                  <>
                    <span className="text-2xl text-burgundy font-medium">{formatPrice(product.sale_price)}</span>
                    <span className="text-lg text-espresso/40 line-through">{formatPrice(product.price)}</span>
                    <span className="label text-burgundy text-[9px] bg-burgundy/10 px-2 py-1">
                      {Math.round((1 - product.sale_price / product.price) * 100)}% OFF
                    </span>
                  </>
                ) : (
                  <span className="text-2xl text-espresso font-medium">{formatPrice(product.price)}</span>
                )}
              </div>

              <p className="text-espresso/60 text-sm font-light leading-relaxed mb-8 max-w-md">{product.description}</p>
              ...
              <div className="mb-6">
                ...
                <p className="text-espresso text-sm font-light">{product.fabric}</p>
              </div>
              ...
              <div className="mb-8">
                ...
                {product.stock === 0 ? (
                  ...
                  <p className="label text-[10px] text-burgundy">ONLY {product.stock} LEFT</p>
                ) : (
              ...
              >
                ...
                <div>
                  <p className="label text-[9px]">HAVE A QUESTION?</p>
                  <p className="label text-[9px]">CHAT WITH US</p>
                </div>
              </a>
          ...
          {relatedProducts.length > 0 && (
            <div className="mt-24 lg:mt-36">
              <div className="mb-12">
                ...
                <h2 className="editorial-heading text-espresso text-4xl lg:text-5xl">More from {product.category}.</h2>
              </div>
